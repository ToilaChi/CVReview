...
- **Kiểm duyệt HR (Tùy chọn):** Nếu hệ thống yêu cầu HR phải đến từ các công ty xác thực, Admin sẽ có quyền phê duyệt/từ chối yêu cầu tạo tài khoản HR của doanh nghiệp mới.
...
- **Quản lý Dead Letter Queue (DLQ):**
    - Khi LlamaParse timeout (>60s) hoặc LLM (Gemini/Groq) bị lỗi kết nối, message sẽ bị đẩy vào DLQ. Admin có thể xem danh sách các CV bị lỗi này.
    - **Tính năng thao tác:** Nút *"Retry Failed Jobs"* để đẩy các message từ DLQ trở lại Queue chính, hoặc bỏ qua/hủy bỏ các CV bị lỗi không thể phục hồi.

## 3. Cấu hình AI & Quản lý Chi phí (AI & System Configuration)
Do hệ thống phụ thuộc nhiều vào API của bên thứ 3 (Llamaindex, Groq, Gemini), Admin cần kiểm soát chặt chẽ ngân sách và hiệu năng:
- **Quản lý API Keys:** 
    - Giao diện nhập và thay đổi linh hoạt API Key cho **LlamaParse**, **Groq** và **Google Gemini** mà không cần khởi động lại container (có thể lưu config vào DB hoặc Cache).
- **Metric Tiêu thụ (Usage/Cost Metrics):**
    ...
    - Cảnh báo (Alerts) qua giao diện khi tài khoản Groq hoặc Gemini gần hết quota.
...
- **Quản lý Storage (Google Drive / Cloudflare R2):**
