...
public interface RefreshTokenRepository extends JpaRepository<RefreshToken, Long> {
    ...
    // Xoá các token cũ nhất vượt quá giới hạn bằng native query để tránh Race
    // Condition
    // Query này xóa sạch các token thừa, giữ lại đúng `limit` token mới nhất của
    // user.
    @Modifying
    @Query(value = "DELETE FROM refresh_tokens WHERE user_id = :userId AND id NOT IN " +
            "(SELECT id FROM (SELECT id FROM refresh_tokens WHERE user_id = :userId ORDER BY created_at DESC LIMIT :limit) AS keep_tokens)", nativeQuery = true)
